...
module.exports = {
  ...
  async update (req, res) {
    try {
      const { body, params: { animalId }, user: {userTypeId}} = req
      console.log(body)
      let animal = await Animal.findById(animalId)
      if(animal.ong == userTypeId){
        animal = await Animal.findByIdAndUpdate(animalId, body, {new: true}).populate({
          path: 'ong',
          select: 'user -_id',
          populate: {
            path: 'user',
        ...
        throw new Error('No tiene autorización para modificar esta información')
      }
      res.status(200).json({ message: 'Datos actualizados con éxito', animal})
    } catch (error) {
      console.dir(error)
      res.status(400).json({ message: 'No se pudo actualizar la información', error})
    }
  }
};
